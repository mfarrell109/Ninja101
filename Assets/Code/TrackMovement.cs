...
public class TrackMovement : MonoBehaviour {
    ...
    // Whenever the movement direction toggles, notify any event listeners.
    public delegate void DirectionChangeAction(bool newDirectionPositive);
    public event DirectionChangeAction OnDirectionChanged;
    private bool oldDirectionPositive;
    ...
    {
        SetupTrack();
        oldDirectionPositive = Mathf.Cos(Time.time * speed) >= 0;
        gameStarted = true;
    }
    ...
    void Update()
    {
        ...
        float waveDirection = Mathf.Cos(Time.time * speed);
        if ((waveDirection < 0 && oldDirectionPositive) ||
            (waveDirection > 0 && !oldDirectionPositive))
        {
            oldDirectionPositive = !oldDirectionPositive;
            if (OnDirectionChanged != null)
            {
                OnDirectionChanged(oldDirectionPositive);
            }
        }
    }
